...
```
docker/
├── Dockerfile           # Python 3.12 + Sphinx + TeX Live + mermaid-cli
└── docker-compose.yml   # 4 種類のサービス定義（dev / html / pdf / epub）
```

このイメージには、日本語 PDF 用の TeX Live（`tex-gyre` フォント含む）に加え、
Mermaid 図を PDF/ePub 用に画像化する mermaid-cli（Node.js + Chromium）も
同梱されています。そのため `make docker-pdf` だけで、図入りの日本語 PDF を
追加設定なしに生成できます。
